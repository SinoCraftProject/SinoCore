...
public record Position(int x, int y) {
    ...
    private static final Codec<Position> CODEC_LIST = RecordCodecBuilder.create(instance -> instance.group(
            Codec.INT.listOf().fieldOf("position").forGetter(p -> List.of(p.x(), p.y()))
    ).apply(instance, l -> new Position(l.get(0), l.get(1))));

    public static final Codec<Position> CODEC = CodecHelper.withDecodingFallback(CODEC_XY, CODEC_LIST);
